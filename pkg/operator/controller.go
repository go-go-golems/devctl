...
func (c *controller) Doctor(ctx context.Context, request DoctorRequest) (DoctorReport, error) {
	...
	if !snapshot.Exists {
		report.Checks = append(report.Checks, DoctorCheck{
			...
			Scope:   "environment",
			Status:  "ok",
			Summary: "no environment state exists",
		})
		return report, nil
	}
	report.Checks = append(report.Checks, DoctorCheck{
	...
	for _, service := range snapshot.Services {
		status := "ok"
		code := ""
		summary := "service run artifacts are internally consistent"
		if service.Phase == runstate.RunUnknown {
			status = "error"
			code = CodeProcessIdentityMismatch
			summary = "service ownership is unknown"
		}
		report.Checks = append(report.Checks, DoctorCheck{
			Check:   "run",
			Scope:   "service",
